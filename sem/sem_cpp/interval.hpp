...
class interval : protected isubs<_T> {
 public:
  ...
  friend std::enable_if<std::is_constructible<_T, _TM>::value, interval<_T> >
  operator+(const _TM& a, const interval<_T>& b) {
    _T aa = _T(a);
    aa.add(b);
    return aa;
  }

  /*friend interval< _T > operator-( const interval< _T >& a, const interval< _T
  >& b ){ interval< _T > res = a; res.subs(b); return res;
  }*/
