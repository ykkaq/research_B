...
\begin{equation}
  \begin{split}
    DF(\bar{x}) & =
    \begin{bmatrix}
      & \multicolumn{0}{c|}{0} & 1 & \cdots & 1 & \cdots 0 \cdots \\ \cline{2-6}
      & \multicolumn{1}{c|}{\vdots}  &  & \vdots & \\
      & \multicolumn{1}{c|}{\partial_\omega f_k} & \cdots & \partial_{a_j} f_k & \cdots & 0 & \\
      & \multicolumn{1}{c|}{\vdots}                    &        & \vdots &        &
   \end{bmatrix}
  \end{split}
\end{equation}
...
\textcolor{blue}{1.$DF(\bar{x})\simeq A^\dagger$としてみる．}
...
よって，式(\ref{eq:s-def})より，
\begin{equation}
  \begin{split}
    S &= ((I-\Pi_N)ADF(\bar{x}))\mid_{X_2} - (\Pi_N ADF(\bar{x}))\mid_{X_2} (\Pi_N ADF(\bar{x}))^{-1}\mid_{X_1} ((I-\Pi_N)ADF(\bar{x}))\mid_{X_1} \\
    &= I_{X_2} - I_{X_2}(A_N DF^{(N)}(\bar{x}))^{-1}\mid_{X_1} I_{X_2} \\
    &= I_{X_2} - (A_N DF^{(N)}(\bar{x}))^{-1}
  \end{split}
\end{equation}

\textcolor{blue}{2. $\bar{x}$を拡張する．}
\\フーリエ係数をサイズ$N$で打ち切らないようにしてみる．つまり，サイズ$\infty$で考える

\begin{equation}
  \begin{split}
    &ADF(\bar{x}) \\
    &= \begin{bmatrix}
      & A_{\omega,0}^{(N)} & \multicolumn{1}{c|}{A_{a,0}^{(N)}} & 0 & \cdots &  0 & \\
      & A_{\omega,1}^{(N)} & \multicolumn{1}{c|}{A_{a,1}^{(N)}} & & & & \\ \cline{2-6}
      & 0 & \multicolumn{1}{c|}{} & (\lambda_N)^{-1} & & \text{\large{0}}\\
      & \vdots & \multicolumn{1}{c|}{} & & (\lambda_{N+1})^{-1} & & \\
      & 0 & \multicolumn{1}{c|}{} & \text{\large{0}} & & \ddots & \\
    \end{bmatrix}
    \begin{bmatrix}
      & 0 & 1 & \cdots & \cdots & \multicolumn{1}{c|}{} & \cdots & \\
      & \vdots & & \vdots & & \multicolumn{1}{c|}{} & \\
      & \partial_w f_k & \cdots & \partial_{a_j} f_k &  \cdots & \multicolumn{1}{c|}{} & \cdots & \\
      & \vdots & & \vdots & & \multicolumn{1}{c|}{} & & \\ \cline{2-7}
      & \vdots & & \vdots & & \multicolumn{1}{c|}{} & \ddots &
    \end{bmatrix}\\
    &=
    \begin{bmatrix}
      A_N DF^{(N)} & 0 \\
