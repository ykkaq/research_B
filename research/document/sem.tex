...
\begin{comment}
  \section{radii polynomalの零点探索の精度保証}
...
\begin{dfn}[作用素$A$の定義]
  Banach空間$X=\mathbb{C} \times l^1$からBanach空間$Y=\mathbb{C} \times l^1$とし，
\end{dfn}
...
従来の手法では，定理\ref{dfn:Banach空間X-重み付きl1空間}よりBanach空間$X$を$X:=\mathbb{C} \times l^1_\nu$と定義していた．許容重みが付加しているため，節\ref{sct:Y0,Z0,Z1,Z2の評価}の評価式のノルムの値が大きくなり，精度が低下する問題があった．

radii-polynomal approachの精度を向上させるため，Banach空間$X$を$X:=\mathbb{C} \times l^1$とし，許容重みを付加せず定義したい．

そこで，radii-polynomal approachにおける，$Y_0$の評価式
\begin{equation}
  \label{eq:y0}
  \|AF(\bar{x})\|_X \leq Y_0
\end{equation}
