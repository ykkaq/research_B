...
function powerconvfourier(a::Vector{Complex{Interval{T}}}, p) where {T}
  ...
  ic_ext = fftshift(verifyfft(ib_ext, 1)) * length_ia_ext^(p - 1) #sign = 1 : fft

  # return ic_ext�,ic_ext�
  return ic_ext[L+N+1:end-N-L+1], ic_ext[L+p:end-(L+p-2)] # return (truncated,full) version
end