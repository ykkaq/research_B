...
\begin{proof}
  ...
  \begin{enumerate}
    ...
    \item $u$が式(\ref{equ:Banachの不動点定理})を満たす真の解$u^*$と一致することを示す(解の存在性)．
    \item 真の解$u^*$が$M$内で一意であることを示す．
  \end{enumerate}

  \textbf{1}

  式(\ref{equ:反復法による点列})より，
  \begin{equation}
  ...
  \end{equation}
  となる定数$k$が存在する．同様に$\|u_{n-1}-u_{n}\|$に式(\ref{equ:反復法による点列})と$A$の縮小写像の性質を使うと最終的に
  \begin{equation}
    \label{equ:点列と縮小写像より}
  ...
  \end{align}
  となる．上の式に式(\ref{equ:点列と縮小写像より})を適用すると
  \begin{align}
  ...
  \end{equation*}
  となるため，$Au$は$Au_n$の極限である．よって式(\ref{equ:反復法による点列})とすると
  \begin{equation*}
    u=Au
  \end{equation*}
  が成立する．とって$u$は式(\ref{equ:Banachの不動点定理})を満たす真の解$u^*$となる．
  ...
  \end{equation*}
  を得る．ここで$0\leq k<1$であるため，不等式を満たすものは$u^*=v^*$の場合のみである．すなわち式(\ref{equ:Banachの不動点定理})を満たす真の解は一意である．
\end{proof}
...
\begin{align}
  f(x) & = \frac{a_0}{2} + \sum_{\infty}^{n=1} (a_n \cos nx + b_n \sin nx)                          \\
...
\begin{align}
  ...
  \int_{-\pi}^{\pi} \sin nx dx = \int_{-\pi}^{\pi} \cos mx dx & = 0
\end{align}

ここで，次のような形式的計算が許されると仮定する．まず，式$(\ref())$
