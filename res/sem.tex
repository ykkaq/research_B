...
\begin{proof}
  ...
  \end{equation*}
  となる．ここで，$2K+m<1$であるため，不等式を満たすものは$\tilde{w_1}=\tilde{w_2}$の場合のみである．すなわち，不動点方程式$w=\mathcal{T}(w)$を満たす解は$\overline{B\left(\bar{x},\rho\right)}$内に一意である．よって，作用素方程式$F(x)=0$を満たす解が$\overline{text{B}(\bar{x},\frac{2\eta}{1-\delta})}$内に一意に存在する．
