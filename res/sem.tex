...
\begin{description}
  ...
  \item[フーリエ級数との関係]
    ...
    \begin{equation*}
      c_k \approx \frac{1}{2M-2} \sum_{j=0}^{2M-3} f(x_j) \exp\left(-\pi i \frac{kj}{M-1}\right),\ \left(\theta_j = \frac{\pi j}{M-1},\ h = \frac{\pi}{M-1} ,\ |k|<M\right)
    \end{equation*}
...
\begin{equation*}
  u_1(t)u_2(t) = \sum_{k\in\mathbb{Z}} \left(a^{(1)} * a^{(2)}_k \exp{(ik   \omega t)}\right)
\end{equation*}
...
\end{equation*}
を離散畳み込みという．スペクトル法の非線形項の計算，$\mathrm{i.e.},u_1,u_2$のような周期関数が$(\in\mathbb{N})$個あったとき，
\begin{equation*}
