...
%DIF DEL old/old.tex   Wed Jan 15 15:06:10 2025
%DIF ADD Outline.tex   Wed Jan 15 15:05:25 2025
...
\begin{equation}
  ...
  \right)
\end{equation}
定義した作用素を用いて，\DIFdelbegin \DIFdel{$\| AF(\bar{x}) \| \leq Y_0$}\DIFdelend \DIFaddbegin \DIFadd{$DF(\bar{x})^{-1}$の全単射性}\DIFaddend を\DIFdelbegin \DIFdel{計算す}\DIFdelend \DIFaddbegin \DIFadd{確かめ}\DIFaddend る．$\Pi_N$を射影演算子とし，
\begin{equation}
  \begin{split}
    &\| I_{X_2} - S\|\\
    &= \DIFaddbegin \DIFadd{\| }\DIFaddend \left( \left( I-\Pi_N \right) ADF ( \bar{x} ) \right)|_{X_2}  \\
    &\quad -(\left( I-\Pi_N \right) ADF( \bar{x} ))|_{X_1} \left((\Pi_N ADF(\bar{x}))|_{X_1}\right)^{-1}\\
    &\qquad ((I-\Pi_N)ADF(\bar{x}))|_{X_2} \DIFaddbegin \DIFadd{\| }\DIFaddend \\
    & < 1
  \end{split}
\end{equation}
となれば，\DIFdelbegin \DIFdel{ノルム空間は$l_1$空間}\DIFdelend \DIFaddbegin \DIFadd{$DF(\bar{x})^{-1}$の全単射性}\DIFaddend で\DIFdelbegin \DIFdel{定義可能で}\DIFdelend ある．
...
実験では，フーリエ係数\DIFdelbegin \DIFdel{列}\DIFdelend の\DIFdelbegin \DIFdel{サイズ}\DIFdelend \DIFaddbegin \DIFadd{次数}\DIFaddend を変更した際のノルム値を検証した．検証結果を\cref{tab:norm-num}に示す．結果より，ノルム値が1より小さいことがわかることから，\rad{}において無限次元ガウスの消去法による計算手法が有効\DIFaddbegin \DIFadd{可能}\DIFaddend であることがわかる．

\begin{table}[htbp]
  \centering
  \caption{フーリエ\DIFdelbeginFL \DIFdelFL{サイズ}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{係数の次数の変更}\DIFaddendFL とノルム値\DIFaddbeginFL \DIFaddFL{の比較}\DIFaddendFL }
  \label{tab:norm-num}
  \begin{tabular}{c||c}
    \DIFdelbeginFL \DIFdelFL{フーリエサイズ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{次数 }\DIFaddendFL & $\| I_{X_2}-S \|$ \\ \hline
